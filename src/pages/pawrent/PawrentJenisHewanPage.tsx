...
const fetchJenisHewan = async (token: string) => {
  const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/jenis-hewan/public/list`, {
...
const PawrentJenisHewanPage = () => {
  const { token } = useAuth();
  const [searchQuery, setSearchQuery] = useState("");
  const { data: jenisList, isLoading } = useQuery({
    queryKey: ["jenis-hewan-pawrent"],
  ...
  return (
    <DashboardLayout title="Jenis Hewan" showBackButton={true} backTo="/pawrent/dashboard">
      <Card className="mb-6">
        <CardHeader>
...
export default PawrentJenisHewanPage;