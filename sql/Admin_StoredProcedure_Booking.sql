...
-- ========================================================
-- STORED PROCEDURES BOOKING (CRUD)
-- - Primary key: booking_id
-- - Return row yang dibuat / diupdate
-- ========================================================
...
CREATE PROCEDURE CreateBooking(
    IN p_dokter_id INT,
    IN p_pawrent_id INT,
    IN p_nama_pengunjung VARCHAR(100),
...
BEGIN
    ...
    -- Pastikan dokter ada dan aktif (menggunakan kolom is_active)
    ...
    -- Asumsi: Shift_Dokter.hari_minggu (1..7), jam_mulai/jam_selesai TIME, is_active flag
    ...
    -- Cek konflik booking dengan aturan jarak minimal 30 menit (1800 detik)
    ...
    IF v_conflict > 0 THEN
        SIGNAL SQLSTATE '23000' SET MESSAGE_TEXT = 'Waktu booking bentrok (jarak minimal 30 menit dengan booking lain untuk dokter ini)';
    END IF;
    ...
    IF p_pawrent_id IS NOT NULL THEN
        ...
        IF v_pawrent_conflict > 0 THEN
            SIGNAL SQLSTATE '23000' SET MESSAGE_TEXT = 'Anda sudah memiliki booking berdekatan dengan dokter ini';
        END IF;
    END IF;
...
BEGIN
    ...
    -- Ambil nilai saat ini untuk pengecekan unik apabila ada perubahan waktu/dokter
    SELECT dokter_id, tanggal_booking, waktu_booking
    INTO v_curr_dokter, v_curr_tanggal, v_curr_waktu
    ...
    -- Jika kombinasi dokter+tanggal+waktu berubah, cek konflik
    IF NOT (v_new_dokter <=> v_curr_dokter) OR NOT (v_new_tanggal <=> v_curr_tanggal) OR NOT (v_new_waktu <=> v_curr_waktu) THEN
        SELECT COUNT(*) INTO v_exists
        FROM Booking
        WHERE dokter_id = v_new_dokter
          AND tanggal_booking = v_new_tanggal
          AND waktu_booking = v_new_waktu
          AND booking_id <> p_booking_id;

        IF v_exists > 0 THEN
            SIGNAL SQLSTATE '23000' SET MESSAGE_TEXT = 'Waktu booking untuk dokter ini sudah terpakai';
        END IF;
    END IF;

    -- Jika pawrent/dokter diberikan, validasi keberadaan
    IF p_dokter_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM Dokter WHERE dokter_id = p_dokter_id) THEN
    ...
    SET
        ...
        nama_pengunjung = COALESCE(p_nama_pengunjung, nama_pengunjung),
