...
const AdminKlinikDokterPage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    tanggal_mulai_kerja: "",
    spesialisasi_id: "",
    klinik_id: user?.klinik_id?.toString() || "", // Fixed ke klinik Admin Klinik
  });

  const { data: dokters, isLoading } = useQuery<any[]>({
    queryKey: ["dokters-klinik", user?.klinik_id],
    queryFn: async () => {
      if (!user?.klinik_id) throw new Error("Klinik ID tidak ditemukan");
      const result = await dokterApi.getByKlinik(user.klinik_id, token!);
      return result as any[];
    },
    enabled: !!user?.klinik_id, // Hanya fetch jika klinik_id ada
  });
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["dokters-klinik", user?.klinik_id] });
  ...
  const handleOpenDialog = (dokter?: any) => {
    if (dokter) {
      ...
      setFormData({
        ...
        klinik_id: user?.klinik_id?.toString() || "", // Tetap fixed
      });
    } else {
      ...
      setFormData({
        ...
        tanggal_mulai_kerja: "",
        spesialisasi_id: "",
        klinik_id: user?.klinik_id?.toString() || "",
      });
    }
  ...
  if (isLoading) {
    return (
      <DashboardLayout title="Kelola Dokter Klinik">
  ...
  return (
    <DashboardLayout title="Kelola Dokter Klinik" showBackButton={true}>
      <div className="space-y-6">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              <Stethoscope className="h-5 w-5" />
              Daftar Dokter di Klinik Anda
            </CardTitle>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                {editingDokter
                  ? "Update informasi dokter di klinik Anda"
                  : "Masukkan informasi dokter baru untuk klinik Anda"}
              </DialogDescription>
            </DialogHeader>
...
export default AdminKlinikDokterPage;